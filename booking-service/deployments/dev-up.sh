...
while ! pg_isready -h localhost -p 5432 -U postgres -d room_service; do
...
../bin/goose -dir ./deployments/migrations postgres "host=localhost user=postgres password=postgres dbname=room_service sslmode=disable" up
